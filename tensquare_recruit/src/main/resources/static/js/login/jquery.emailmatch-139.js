...
(function($){
	$.fn.emailMatch= function(settings){
		var defaultSettings = {
			emailTip:"",
			aEmail:["139.com"], //邮件数组  ,"189.cn","qq.com","gmail.com","126.com","163.com","hotmail.com","yahoo.com","yahoo.com.cn","live.com","sohu.com","sina.com"
			wrapLayer:"#loginIDCheck",
		...
		return this.each(function(){
            ...
            $(mailWrap).on("mouseover", "li:not('.first')", function (e) {
                $(e.currentTarget).addClass("current").siblings().removeClass("hover").removeClass("current");
                this.focus();
            });
            $(mailWrap).on("mousedown", "li:not('.first')", function (e) {
                $(e.currentTarget).addClass("current").siblings().removeClass("current");
                enterVal(mailWrap,elem);
                hideList($(mailWrap));
            });
            $(mailWrap).on("click", "li", function (e) {
                enterVal(mailWrap,elem);
                hideList($(mailWrap));
            });
            $(mailWrap).on("mouseout", "li", function (e) {
	...
	function enterVal(oWrap,elem){
		if($(oWrap).is(":visible")){
			 var text = $(oWrap).find("li.current").text();
			 elem.val(text); // 获取高亮内容 到 输入框
		}		
	};
	
	function showList(oElem,w,h,t,l){ // 显示 邮箱框架 并定位.
		oElem.css({"display":"block","z-index":"9999","top":h + t-25,"left":l,"width":w-2,"position":"relative","font-size":"16px","color":"#999"});
	};
